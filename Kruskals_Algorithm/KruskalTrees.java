...
//Edge class to store each edge and vertices
class Edge {
    public int u, v, wgt;
...
//Heap class to sort edges and weight in relation with each vertices
class Heap
{
	private int[] h;
    int N, Nmax;
...
class UnionFindSets
{
    ...
    public UnionFindSets( int V)
    {
        ...
        for (int i = 1; i <= V; i++)// missing lines
			treeParent[i] = i;
	}

    public int findSet( int vertex)
    {   
        if (treeParent[vertex] == vertex)// missing lines
		{
			return vertex;
		}
		else
